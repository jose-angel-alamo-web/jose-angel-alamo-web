  ...
  return (
    <nav
      className={`fixed w-full z-50 transition-all duration-300 ${
        isScrolled ? `${theme.primary} shadow-lg py-2` : "bg-[#1B3A57] py-4"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-3">
          <div
            className={`bg-white transition-all duration-300 shadow-md flex items-center justify-center overflow-hidden
              ${
                isScrolled
                  ? "w-10 h-10 p-0.5 rounded-b-xl rounded-t-sm" 
                  : "w-16 h-16 p-0.5 rounded-b-2xl rounded-t-md" 
              }`}
          >
            <img
              src={logoUrl}
          ...
          <span
            className={`font-serif font-bold text-white tracking-wide transition-all duration-300 ${
